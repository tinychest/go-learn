...
func TestErrIs(t *testing.T) {
	...
	fmt.Println(e1 == e2) // 呃，当然为 false
	fmt.Println(errors.Is(e2, e1)) // false
	...
	fmt.Println(e1 == e2.(MyError2).Unwrap()) // true
	fmt.Println(e1 == errors.New("error 01")) // false

	re := e2.(MyError2).Unwrap()
	// 假如有面试题，地址不相同的两个实例，通过 == 比较，一定为 false 么，如果，不是这样的，举个例子
	fmt.Printf("%p %p %t\n", &e1, &re, e1 == re)
	// 其实这个本质是
	fmt.Println(MyError1{} == MyError1{})
}

func TestErrIs2(*testing.T) {
	// 再仔细想想，这个题，问的有问题
	var i1 = 1
	var i2 = 1
	fmt.Printf("%p %p %t\n", &i1, &i2, i1 == i2)
	...
	var ee = e
	fmt.Println(e == ee) // true

	// 见 errors.New 源码，点1 - 摸得到的实例，点2 -地址
}

func TestErrIs3(*testing.T) {
	// 继续引出（本以为是不相等的，没事了）
	fmt.Println(MyError1{"123"} == MyError1{"123"})
