...
func basicTest() {
	...
	go func() {
		intValue := 1
		// fmt.Printf("向通道 1 发送值：%d\n", intValue)
		intChannel1 <- intValue
	}()
	go func() {
		intValue := 1
		// fmt.Printf("向通道 2 发送值：%d\n", intValue)
		intChannel2 <- intValue
	}()
	go func() {
		intValue := 1
		// fmt.Printf("向通道 3 发送值：%d\n", intValue)
	...
	case receiveInt := <-intChannel1:
		fmt.Printf("从通道1读取值：%d\n", receiveInt)
	case receiveInt := <-intChannel2:
		fmt.Printf("从通道2读取值：%d\n", receiveInt)
	case receiveInt := <-intChannel3:
		fmt.Printf("从通道3读取值：%d\n", receiveInt)
	default:
