...
// 总结一下：
// 1、从 map 的机理来说，因为底层存储 value 的位置会因为底层扩容而发生改变，也就是上面说的“不可寻址”，所以通过 map 取得的值就只是值，也就是当前指定键对应的值，可以当作一个常量来理解，无法进行 & 的操作
...
// 没错，是没有任何无法根据键取值的意思，键对应的值是肯定可以取到的，不然 map 设计了个寂寞
// - 不可寻址强调寻到的值的地址是不确定的，换句话说取到的值是没有意义的，是可能会改变的，所以说不要进行取地址的操作，也不要进行赋值的操作（同理去理解，无法理解，上例就是没有理解），GO 的编译器也直接禁止了这个操作

// 2、接着说，如果如果 map 的值是指针类型或是具有表示地址含义的（array、slice、chan、map、Ptr），虽说不能取值，但是你能通过这些地址去改变你想改变的，来满足逻辑需求
...
func testChangeValue1() {
	...
	// 结论：可以修改，但是不可能实现修改，这和数组底层没有任何关系，那就是一块内存中的值，确定范围都是切片的属性
	temp := testMap[0]
