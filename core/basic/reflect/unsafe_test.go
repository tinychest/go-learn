...
如果想了解 unsafe 包，应该直接去看原文，这里的记录仅仅为了促进理解
...
[地址值]
Go 中使
用 uintptr 类型来表示一个指针指向的地址值，本质上就是一个整数值
...
[如何安全的使用非类型安全的指针]
详见 unsafe 包的注释，给出了 6 个例子，详见下面的 practiceTest
*/
...
func conceptTest(t *testing.T) {
	...
	// *任意类型 ←→ unsafe.Pointer
	p := unsafe.Pointer(sPtr) // 可以包裹任意 * 类型
	_ = (*int)(p)             // unsafe.Pointer 可以被断言成任何类型（即使显而易见不对，也不会基于提示，所以是 unsafe）

	// unsafe.Pointer → uintptr →（会有警告） unsafe.Pointer
	u := uintptr(p)       // unsafe.Pointer 可以转成地址值
	p = unsafe.Pointer(u) // 将任意一个地址值，包装成 unsafe.Pointer 去使用，编译器会给出警告
}

func practiceTest(t *testing.T) {
	...
	// TODO 虽然安全，但是不推荐在用户代码中使用，应该使用文案结尾 模式六 样例
...
func otherTest(t *testing.T) {
	// 样例3 struct_field_set_test.go
	// 样例4 []Interface ←→ []Struct
	...
	eats = *(*[]Eat)(unsafe.Pointer(&eaters))
	eaters = *(*[]Eater)(unsafe.Pointer(&eats))
}

func sizeOfTest(t *testing.T) {
	// int 8
...
func addTest(t *testing.T) {
	p := [4]int{0: 0, 1: 1, 2: 2, 3: 3}
	eleSize := int(unsafe.Sizeof(p[0]))
	p3 := &p[3]
	up3 := unsafe.Pointer(p3)

	p2 := (*int)(unsafe.Add(up3, -1*eleSize))
	t.Log(*p2) // 2

	_ = unsafe.Add(up3, eleSize) // 引用了未知的内存块，实际运行都不一定报错，这就是 unsafe
}

func sliceSimpleTest(t *testing.T) {
	pn := unsafe.Slice((*int)(nil), 0)
	t.Log(pn == nil) // true

	p := [4]int{0: 0, 1: 1, 2: 2, 3: 3}
	p1 := &p[1]

	// 从 p1 为起始地址，向后取 3 个元素（cap），实际 2（len）
	s := unsafe.Slice(p1, 3)[:2]
	t.Log(s)              // [1 2]
	t.Log(len(s), cap(s)) // 2 3

	_ = unsafe.Slice(p1, 4) // 引用了未知的内存块，实际运行都不一定报错，这就是 unsafe
}
