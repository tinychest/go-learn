...
// 二、n sender - 1 receiver：所有引用通道的协程都结束了，这个通道会被作为垃圾回收掉，需要额外创建一个通道，这样才不会违反原则
//  sender   当符合条件时，直接关闭额外通道
//  receiver 从额外通道读，读到就退出
// 三、m sender - n receiver：思想同上，但是说将关闭额外通道的逻辑无论是放到 sender 还是 receiver 都会违反原则，所以就再起一个协程作为中间者，负责关闭额外通道
