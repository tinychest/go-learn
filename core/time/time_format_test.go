...
func TestTimeFormat(t *testing.T) {
	dateAndTimeString1 := now(time_format.FmtDateTime)
	dateAndTimeString2 := now(time_format.FmtDateTime)
	dateString := now(time_format.FmtDate)
	timeString := now(time_format.FmtTime)
